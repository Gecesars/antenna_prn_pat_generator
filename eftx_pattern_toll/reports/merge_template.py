...
def apply_template_underlay(
    ...
    for idx, page in enumerate(content_reader.pages, start=1):
        ...
        # Merge template as underlay directly into each content page.
        # Using over=False prevents the template from covering report graphics.
        content_page = copy.deepcopy(page)
        template_page = copy.deepcopy(template_reader.pages[template_idx])
        content_page.merge_page(template_page, over=False)
