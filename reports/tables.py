...
def build_table_for_pdf(
    ...
    *,
    ...
    for c in range(width):
        col_vals = [data[r][c] for r in range(1, len(data))]
        if not col_vals:
            continue
        numeric_hits = sum(1 for x in col_vals if _is_numeric_text(x))
        if numeric_hits >= max(2, int(0.6 * len(col_vals))):
            style.append(("ALIGN", (c, 1), (c, -1), "RIGHT"))
        else:
            style.append(("ALIGN", (c, 1), (c, -1), "LEFT"))
