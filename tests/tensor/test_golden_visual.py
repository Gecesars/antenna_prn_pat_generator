...
def test_golden_pipeline(tmp_path):
    ...
    pipe = Pipeline([GaussianBlurOp(0.7), ContrastOp(1.06)], name="golden")
    out = pipe.execute(img, device="cpu", use_cache=False)

    actual_path = tmp_path / "out_actual.png"
    ...
    if actual.shape != expected.shape:
        raise AssertionError(f"Golden size mismatch: {actual.shape} vs {expected.shape}")
    ...
    if psnr < 45.0:
        ...
        raise AssertionError(f"Golden mismatch PSNR={psnr:.2f} dB | actual={actual_path} | diff={diff_path}")
